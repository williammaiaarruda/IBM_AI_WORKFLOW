...
if __name__ == "__main__":
    ...
    print("CsLib: Initializing Procedure")
    run_start = time.time() 
    data_dir = os.path.join(".","data","cs-train")
    print("CsLib: Fetching Data")

    ts_all = fetch_ts(data_dir,clean=False)

    m, s = divmod(time.time()-run_start,60)
    h, m = divmod(m, 60)
    print("CsLib: Fetch Completed. Total Load Time :","%d:%02d:%02d"%(h, m, s))

    for key,item in ts_all.items():
        print("CsLib:", key,item.shape)

    print("CsLib: Finished Procedure")